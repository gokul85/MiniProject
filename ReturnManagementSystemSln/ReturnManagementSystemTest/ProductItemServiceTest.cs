...
namespace ReturnManagementSystemTest
{
    public class ProductItemServiceTest
    {
        ...
        public async Task UpdateProductItemStatusTestSuccess(string sn, string status)
        {
            ...
            var result = await _productItemService.UpdateProductItemStatus(sn, status);
        ...
        {
            Assert.ThrowsAsync<ObjectNotFoundException>(async () => await _productItemService.UpdateProductItemStatus("SN999", "Available"));
        }
    }
}
